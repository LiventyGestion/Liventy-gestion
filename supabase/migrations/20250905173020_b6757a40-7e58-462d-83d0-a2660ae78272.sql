...
-- =============== POLÍTICAS RLS PARA INCIDENCIAS (actualizar) ===============
...
FOR SELECT USING (
  ...
  EXISTS (
    ...
    WHERE pr.id = "Incidencias".propiedad_id
...
FOR INSERT WITH CHECK (
  ...
  (sender_user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.chat_threads t WHERE t.id = thread_id AND t.user_id = auth.uid()))
