-- Security Enhancement: Implement Least Privilege Data Access
...
BEGIN
    ...
    IF TG_TABLE_NAME IN ('leads', 'calculadora_resultados', 'chatbot_conversations') THEN
        -- Log the access attempt (this will appear in Postgres logs)
        RAISE NOTICE 'Sensitive data access: Table=%, User=%, Operation=%, Time=%', 
            TG_TABLE_NAME, 
...
CREATE TRIGGER audit_leads_access
    BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.leads
    FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();

DROP TRIGGER IF EXISTS audit_calculadora_access ON public.calculadora_resultados;
CREATE TRIGGER audit_calculadora_access
    BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.calculadora_resultados
    FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();

DROP TRIGGER IF EXISTS audit_chatbot_access ON public.chatbot_conversations;
CREATE TRIGGER audit_chatbot_access
    BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.chatbot_conversations
    FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();