-- Security Enhancement Phase 1 & 2: Critical and Medium Priority Fixes

-- =============================================
-- PHASE 1: STRENGTHEN USER DATA POLICIES (CRITICAL)
-- =============================================

-- Add enhanced logging for Usuarios table access attempts
CREATE OR REPLACE FUNCTION public.log_usuarios_access_detailed()
RETURNS TRIGGER AS $$
BEGIN
    -- Log any suspicious access attempts with detailed context
    IF TG_OP = 'SELECT' AND auth.uid() IS NULL THEN
        PERFORM public.log_security_event(
            'anonymous_usuarios_access_blocked',
            NULL,
            NULL,
            NULL,
            jsonb_build_object(
                ...
                'operation', TG_OP,
                'timestamp', NOW(),
                'blocked_reason', 'anonymous_access_not_allowed'
            ),
            'high'
        );
    END IF;
    ...
    IF TG_OP IN ('SELECT', 'UPDATE') AND auth.uid() IS NOT NULL THEN
        IF TG_OP = 'SELECT' AND OLD.id != auth.uid() THEN
            PERFORM public.log_security_event(
...
-- Create trigger for detailed Usuarios access logging
...
CREATE TRIGGER usuarios_access_detailed_monitor
    BEFORE SELECT OR UPDATE OR DELETE ON public."Usuarios"
    FOR EACH ROW
...
-- Add comprehensive audit logging for Pagos table
...
BEGIN
    ...
    -- Log all operations on payment data
...
CREATE TRIGGER pagos_audit_log
    AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public."Pagos"
    FOR EACH ROW
